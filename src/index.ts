...
function rotateObjectZ(obj: any, af: number, totalIterations: number) {
  ...
  Rota3D.initRotate2(center, 0, 0, af * Math.PI / 180); // Girar en dirección a Z
...
function rotateObjectX(obj: any, af: number, totalIterations: number) {
   // Calcular el centro del objeto
   let center = calculateCenter(obj);

   // Inicializar la matriz de rotación utilizando el centro como punto de origen
   Rota3D.initRotate2(center, Math.PI / 2, 0, af * Math.PI / 180); // Girar en dirección a X
...
function calculateCenter(obj: any): Point3D {
  ...
  const numVertices = 376; // Número total de vértices del objeto
...
document.getElementById('pza1Izq').addEventListener('click', ()=>{
  rotateObjectX(obj, 20, 250)
}, false);
document.getElementById('pza1Der').addEventListener('click', ()=>{rotateObjectY(obj, 20, 250)}, false);
document.getElementById('pza12Izq').addEventListener('click', ()=>{
  rotateObjectZ(obj, 20, 250)
}, false)
;
